...
fn replace_control_char(c: char) -> char {
	match c as u32 {
		0x00..=0x1F => {
...
impl MemoryWindowState {
	pub(super) fn show(&mut self, core: &mut Core, mem: &mut Memory, ctx: &egui::Context) {
		use MemoryPosition::*;
		use MemoryInterpretation::*;
		...
		egui::Window::new("Memory Monitor")
			...
			let looked = {
				match self.look {
					...
					LastException => (core.inner.cp0[Cp0Register::ExPC] >> 2).saturating_sub(3) << 2,
					Position(p) => p,
				}
			};
			...
			ui.horizontal(|ui| {
				ui.menu_button("View", |ui| {
					...
					ui.label("Use the radio buttons at the top-left of the window to select which CPU core to focus on.");
				});
			});
			...
			egui::ScrollArea::vertical()
				.auto_shrink([false; 2])
				.show_rows(ui, row_height, row_num,
			|ui, row_range| {
			...
			|ui| {
				for row in row_range {
					...
					let epc = core.inner.cp0[Cp0Register::ExPC];
					
					let line_highlight = match addr {
						_ if Some(addr) == delay_slot => Some(("→", egui::Color32::WHITE, egui::Color32::from_rgb(0x45, 0x85, 0x88))),
					...
					ui.add_sized([14.0, ui.available_height()], egui::Label::new(
						if let Some((label, color, bg_color)) = line_highlight {
							egui::RichText::new(label).color(color).background_color(bg_color)
						} else {
							egui::RichText::new("")
						}
					));
					
					let brk = core.breakpoints.iter()
						.enumerate().find(|&(_, brk)| *brk == addr);
